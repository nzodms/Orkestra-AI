...
import {
  generateCollectionSeo, generateMetaVariants, generateAltTexts, generateBlogOutline,
  type CollectionSeoResult, type MetaVariant, type BlogOutlineResult,
} from "@/lib/ai/engine";
...
export default function SeoStudioPage() {
  ...
  const [collResult, setCollResult] = useState<CollectionSeoResult | null>(null);
  const [metaResult, setMetaResult] = useState<MetaVariant[] | null>(null);
  const [altResult, setAltResult] = useState<string[] | null>(null);
  const [blogResult, setBlogResult] = useState<BlogOutlineResult | null>(null);
  ...
  async function generate() {
    ...
    const res = await fetch("/api/generate", {
      ...
      body: JSON.stringify({
        ...
        context: {
          brandName: brand.storeName || undefined, niche: brand.niche || undefined, positioning: brand.positioning,
          language: brand.language, collections: brand.collections, productTypes: brand.productTypes,
          primaryKeywords: brand.primaryKeywords, competitors: brand.competitors,
        },
      }),
    });
    ...
    if (data.ok) {
      ...
      addGeneration({
        id: crypto.randomUUID(), type: "seo-product", title: `Fiche SEO — ${form.productName || "Produit"}`,
        store: brand.storeName || "Boutique", createdAt: new Date().toISOString(),
        models: providers.length ? providers : ["openai"], status: "completed", score: data.result.seoScore,
        preview: "Titre, description HTML, FAQ, meta, tags et mots-clés générés.",
      } as GenerationRecord);
    }
  }
  ...
  return (
    <>
      <PageHeader
        title="SEO Studio"
        description="Le studio de production : générez des contenus SEO prêts à publier (fiches, meta, FAQ, alt, collections, blog), alignés sur votre niche et votre ton — naturels et conformes Google Merchant."
      />

      {/* Chips de contexte issus du scan */}
      {analysis && (
        <div className="mb-4 flex flex-wrap gap-2">
          {!!analysis.priorityProducts?.length && (
            <button onClick={() => setActive("product")} className="inline-flex items-center gap-1.5 rounded-full border border-[var(--border)] bg-[var(--surface)] px-3 py-1 text-xs transition hover:border-brand-300">
              <Package className="h-3.5 w-3.5 text-brand-500" /> {analysis.priorityProducts.length} produits prioritaires
            </button>
          )}
          {!!analysis.metrics?.missingMetaDescriptions && (
            <button onClick={() => setActive("meta")} className="inline-flex items-center gap-1.5 rounded-full border border-[var(--border)] bg-[var(--surface)] px-3 py-1 text-xs transition hover:border-brand-300">
              <Tag className="h-3.5 w-3.5 text-amber-500" /> {analysis.metrics.missingMetaDescriptions} meta manquantes
            </button>
          )}
          {!!(analysis.catalogStats?.imagesNoAlt ?? analysis.metrics?.imagesWithoutAlt) && (
            <button onClick={() => setActive("alt")} className="inline-flex items-center gap-1.5 rounded-full border border-[var(--border)] bg-[var(--surface)] px-3 py-1 text-xs transition hover:border-brand-300">
              <ImageIcon className="h-3.5 w-3.5 text-amber-500" /> {analysis.catalogStats?.imagesNoAlt ?? analysis.metrics?.imagesWithoutAlt} images sans alt
            </button>
          )}
          {!!brand.collections.length && (
            <button onClick={() => setActive("collection")} className="inline-flex items-center gap-1.5 rounded-full border border-[var(--border)] bg-[var(--surface)] px-3 py-1 text-xs transition hover:border-brand-300">
              <FolderOpen className="h-3.5 w-3.5 text-brand-500" /> {brand.collections.length} collections
            </button>
          )}
        </div>
      )}
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        ...
        <Card>
          {active === "product" && (
            <>
              ...
              <div className="space-y-4">
                ...
                <Field label="Bénéfices"><input className="input" value={form.benefits} onChange={(e) => set("benefits", e.target.value)} placeholder="Ex : gain de place, pliable, silencieux" /></Field>
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <Field label="Niveau SEO">
                    <div className="flex gap-1.5">
                      {(["standard", "poussé", "ultra"] as SeoLevel[]).map((l) => (
                        <button key={l} onClick={() => set("level", l)} className={`flex-1 rounded-lg border px-2 py-2 text-xs font-medium capitalize transition ${form.level === l ? "border-brand-500 bg-brand-50 text-brand-700 dark:bg-brand-950" : "border-[var(--border)] text-[var(--text-muted)]"}`}>{l}</button>
                      ))}
                    </div>
                  </Field>
                </div>
                <Button onClick={generate} loading={loading} size="lg" className="w-full" icon={!loading ? <Sparkles className="h-4 w-4" /> : undefined}>
                  {loading ? "Génération en cours…" : "Générer la fiche SEO"}
                </Button>
          ...
          {active === "collection" && (
            <>
              ...
              <Field label="Collection">
                <input className="input" value={collInput} onChange={(e) => setCollInput(e.target.value)} placeholder={cols[0]} list="ork-cols2" />
                <datalist id="ork-cols2">{cols.map((c) => <option key={c} value={c} />)}</datalist>
              </Field>
              <Button onClick={() => setCollResult(generateCollectionSeo({ collection: collInput || cols[0], ...nicheCtx, others: cols }))} className="mt-4 w-full" icon={<Wand2 className="h-4 w-4" />}>Générer le contenu collection</Button>
            </>
          )}
          ...
          {active === "meta" && (
            <>
              ...
              <Field label="Type de page">
                <div className="flex gap-1.5">
                  {(["produit", "collection", "accueil"] as const).map((t) => (
                    <button key={t} onClick={() => setMetaType(t)} className={`flex-1 rounded-lg border px-2 py-2 text-xs font-medium capitalize transition ${metaType === t ? "border-brand-500 bg-brand-50 text-brand-700 dark:bg-brand-950" : "border-[var(--border)] text-[var(--text-muted)]"}`}>{t}</button>
                  ))}
                </div>
              </Field>
              ...
              <Button onClick={() => setMetaResult(generateMetaVariants({ subject: metaSubject || (metaType === "accueil" ? brand.niche || preset.label : cols[0]), type: metaType, ...nicheCtx }))} className="mt-4 w-full" icon={<Wand2 className="h-4 w-4" />}>Générer 3 variantes</Button>
            </>
          )}
          ...
          {active === "alt" && (
            <>
              ...
              <Button onClick={() => setAltResult(generateAltTexts({ subject: altSubject || preset.sampleProduct, niche: brand.niche, count: 5 }))} className="mt-4 w-full" icon={<Wand2 className="h-4 w-4" />}>Générer les alt text</Button>
            </>
          )}
          ...
          {active === "blog" && (
            <>
              ...
              <div className="mt-4"><Field label="Collection à mailler">
                <input className="input" value={blogColl} onChange={(e) => setBlogColl(e.target.value)} placeholder={cols[0]} list="ork-cols3" />
                <datalist id="ork-cols3">{cols.map((c) => <option key={c} value={c} />)}</datalist>
              </Field></div>
              <Button onClick={() => setBlogResult(generateBlogOutline({ topic: blogTopic, collection: blogColl || cols[0], ...nicheCtx }))} className="mt-4 w-full" icon={<Wand2 className="h-4 w-4" />}>Générer le plan d&apos;article</Button>
            </>
          )}
        </Card>
        ...
        <div>
          {active === "product" && (
            <>
              {!result && !loading && <EmptyResult label="Votre fiche SEO apparaîtra ici" />}
              {loading && <SkeletonResult />}
              {result && (
                <>
                  <div className="mb-2 flex items-center gap-2 text-xs">
                    {meta?.live ? <Badge tone="good">OpenAI live</Badge> : <Badge tone="neutral">Mode démo</Badge>}
                    <span className="text-[var(--text-muted)]">{meta?.live ? `Généré avec OpenAI · ${meta.model}` : `Fiche simulée${meta?.fallbackReason ? ` · ${meta.fallbackReason}` : ""}`}</span>
                  </div>
          ...
          {active === "alt" && (altResult ? <AltResult alts={altResult} subject={altSubject || preset.sampleProduct} /> : <EmptyResult label="Vos alt text apparaîtront ici" />)}
...
function SkeletonResult() {
  return <Card className="min-h-[360px] space-y-3">{Array.from({ length: 8 }).map((_, i) => <div key={i} className="h-4 animate-pulse rounded bg-ink-100 dark:bg-ink-900" style={{ width: `${60 + Math.random() * 40}%` }} />)}</Card>;
}
...
function Block({ title, value, mono }: { title: string; value: string; mono?: boolean }) {
  return (
    <div>
      <div className="mb-1 flex items-center justify-between"><span className="text-xs font-semibold uppercase tracking-wide text-ink-400">{title}</span><CopyBtn text={value} /></div>
...
function ListBlock({ title, items }: { title: string; items: string[] }) {
  return (
    <div>
      ...
      <div className="mt-2 flex flex-wrap gap-1.5">{items.map((it, i) => <span key={i} className="rounded-lg bg-ink-100 px-2 py-1 text-xs dark:bg-ink-900">{it}</span>)}</div>
    </div>
  );
}
function FaqBlock({ faq }: { faq: { q: string; a: string }[] }) {
  return (
    <div>
      ...
      <div className="mt-2 space-y-2">{faq.map((f, i) => (
        <div key={i} className="rounded-xl border border-[var(--border)] p-3"><div className="text-sm font-medium">{f.q}</div><div className="mt-1 text-sm text-[var(--text-muted)]">{f.a}</div></div>
      ))}</div>
    </div>
  );
}
function WhereRow({ where, addQ }: { where: string; addQ: string }) {
  return (
    <div className="flex flex-wrap items-center gap-2 rounded-xl border border-[var(--border)] bg-[var(--bg)] p-3 text-xs">
      <MapPin className="h-3.5 w-3.5 shrink-0 text-brand-500" />
      <span className="font-mono text-[11px] text-[var(--text-muted)]">{where}</span>
      <Link href={assistantLink(addQ)} className="ml-auto"><Button variant="ghost" size="sm" icon={<ArrowRight className="h-3.5 w-3.5" />}>Où l&apos;ajouter ?</Button></Link>
    </div>
  );
}
  ...
  return (
    <Card className="animate-fade-in space-y-5">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold">Fiche générée</h3>
        <div className="flex gap-3 text-xs">
          <span className="flex items-center gap-1.5"><span className="text-[var(--text-muted)]">SEO</span><Badge tone="good">{result.seoScore}</Badge></span>
          <span className="flex items-center gap-1.5"><span className="text-[var(--text-muted)]">Conversion</span><Badge tone="good">{result.conversionScore}</Badge></span>
        </div>
      </div>
      <Block title="Titre optimisé" value={result.optimizedTitle} />
      <div className="grid gap-4 sm:grid-cols-2">
        <Block title="Meta title" value={result.metaTitle} />
        <Block title="Handle" value={result.handle} mono />
      </div>
      <Block title="Meta description" value={result.metaDescription} />
      {(result.productType || result.parentCollection) && (
        <div className="grid gap-4 sm:grid-cols-2">
          {result.productType && <Block title="Product type" value={result.productType} />}
          {result.parentCollection && <Block title="Collection parente" value={result.parentCollection} />}
        </div>
      )}
      <div>
        <div className="mb-1.5 flex items-center justify-between"><span className="text-xs font-semibold uppercase tracking-wide text-ink-400">Description longue (HTML Shopify)</span><CopyBtn text={result.longDescriptionHtml} /></div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        {!!result.tags?.length && <ListBlock title="Tags recommandés" items={result.tags} />}
        <ListBlock title="Alt text images" items={result.imageAltTexts} />
      </div>
      <FaqBlock faq={result.faq} />
      {result.merchantNote && (
        <div className="rounded-xl bg-emerald-50 p-3 text-xs text-emerald-800 dark:bg-emerald-950/30 dark:text-emerald-200">
          <span className="font-semibold">Google Merchant : </span>{result.merchantNote}
        </div>
      )}
      <div className="flex flex-wrap gap-2 border-t border-[var(--border)] pt-4">
        <Link href={assistantLink("Où ajouter une fiche produit (description, meta, tags, type) dans Shopify ?")}><Button variant="secondary" size="sm" icon={<MapPin className="h-3.5 w-3.5" />}>Où l&apos;ajouter dans Shopify ?</Button></Link>
        <Link href={councilLink("seo", "Améliore encore cette fiche produit et propose des idées de maillage.")}><Button variant="outline" size="sm" icon={<Sparkles className="h-3.5 w-3.5" />}>Demander à AI Council</Button></Link>
      </div>
    </Card>
  );
}
...
function CollectionResult({ r }: { r: CollectionSeoResult }) {
  return (
    <Card className="animate-fade-in space-y-5">
      ...
      <div>
        <div className="mb-1.5 flex items-center justify-between"><span className="text-xs font-semibold uppercase tracking-wide text-ink-400">Description de collection (à coller)</span><CopyBtn text={r.descriptionHtml} /></div>
        <pre className="max-h-52 overflow-auto rounded-xl bg-ink-950 p-3 text-xs text-ink-100"><code>{r.descriptionHtml}</code></pre>
      </div>
      ...
      {variants.map((v, i) => (
        <div key={i} className="rounded-xl border border-[var(--border)] p-3.5">
          <div className="mb-1 flex items-center justify-between">
            <span className="text-[11px] font-semibold uppercase tracking-wide text-brand-600 dark:text-brand-300">Variante {i + 1}</span>
            <Badge tone={v.titleLen <= 60 ? "good" : "warn"}>{v.titleLen} car.</Badge>
          </div>
          ...
          <div className="mt-2 flex items-center justify-between gap-2 border-t border-[var(--border)] pt-2">
            <span className="min-w-0 break-words text-sm text-[var(--text-muted)]">{v.metaDescription}</span><CopyBtn text={v.metaDescription} />
          </div>
  ...
  return (
    <Card className="animate-fade-in space-y-3">
      ...
      <div className="space-y-1.5">
        {alts.map((a, i) => (
          <div key={i} className="flex items-center justify-between gap-2 rounded-lg border border-[var(--border)] bg-[var(--bg)] px-2.5 py-1.5 text-sm">
            <span className="min-w-0 break-words">{a}</span><CopyBtn text={a} />
          </div>
        ))}
      </div>
...
function BlogResult({ r }: { r: BlogOutlineResult }) {
  return (
    <Card className="animate-fade-in space-y-4">
      <div>
        <div className="mb-1 flex items-center justify-between"><span className="text-xs font-semibold uppercase tracking-wide text-ink-400">Titre</span><CopyBtn text={r.title} /></div>
        <div className="rounded-xl border border-[var(--border)] bg-[var(--bg)] px-3.5 py-2.5 text-sm font-medium">{r.title}</div>
      </div>
      <div className="flex flex-wrap items-center gap-2 text-xs">
        <Badge tone="brand">Mot-clé : {r.keyword}</Badge>
        <Badge tone="neutral">{r.intent}</Badge>
      </div>
      ...
      <div className="rounded-xl bg-brand-50 p-3 text-xs dark:bg-brand-950/40">
        <div className="text-brand-800 dark:text-brand-200"><span className="font-semibold">Maillage : </span>{r.internalLink}</div>
        <div className="mt-1 text-brand-800 dark:text-brand-200"><span className="font-semibold">CTA : </span>{r.cta}</div>
      </div>
      <div className="flex flex-wrap gap-2 border-t border-[var(--border)] pt-4">
        <Link href={councilLink("seo", `Rédige l'article complet « ${r.title} » avec le plan proposé.`)}><Button variant="secondary" size="sm" icon={<Sparkles className="h-3.5 w-3.5" />}>Rédiger avec AI Council</Button></Link>
        <Link href={assistantLink("Où publier un article de blog dans Shopify ?")}><Button variant="outline" size="sm" icon={<MapPin className="h-3.5 w-3.5" />}>Où le publier ?</Button></Link>
      </div>
    </Card>
  );
}
