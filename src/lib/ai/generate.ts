...
const MODE_ROLE: Record<CouncilMode, string> = {
  seo: "tu es un CONSULTANT SEO Shopify SENIOR (niveau agence). Tu produis une vraie stratégie : on-page collections (pages business), SEO produits, cocon de contenu/blog, maillage interne, mots-clés courte+longue traîne, avec chemins Shopify exacts et metas premium. Vocabulaire professionnel mais accessible. Jamais de checklist générique ni de formules faibles type « Achetez maintenant ».",
...
function modeStructure(mode: CouncilMode): string {
  const seo =
    "Réponds comme un CONSULTANT SEO Shopify SENIOR (niveau agence), pas une checklist. Vocabulaire pro mais compréhensible (optimisation on-page, page business, requête transactionnelle, intention de recherche, longue traîne, cocon sémantique, maillage interne, ancres optimisées, H1 unique, snippets). Sépare clairement : A) SEO collections, B) SEO produits, C) SEO éditorial/blog, D) maillage, E) plan.\n" +
    "Structure imposée :\n## 1. Résumé du scan\n## 2. ✅ Déjà bon — à NE PAS prioriser (liste les éléments que le scan indique comme corrects : titres faibles=0, tags=100%, meta présentes, pages légales OK — ne recommande pas de les modifier)\n## 3. Priorités par impact (haute/moyenne/basse)\n## 4. SEO des collections (pages business) — pour CHAQUE collection RÉELLE : intention de recherche, mot-clé principal + secondaires + longue traîne, meta title (≤60, premium), meta description (≤155, premium), contenu collectionnel 150–250 mots À METTRE dans la description de collection, structure H1 unique + H2/H3, FAQ 4–6, maillage (ancres), et le chemin Shopify exact (Produits > Collections > Description ; SEO via Aperçu du référencement naturel)\n## 5. SEO des produits — pour CHAQUE produit prioritaire : problème réel, mot-clé cible, correction exacte, alt text, FAQ, action (SEO Studio). N'optimise PAS une description si le scan la dit déjà correcte.\n## 6. SEO éditorial — CALENDRIER de contenu : 8 à 12 idées d'articles, chacune avec titre, intention (guide/informationnel/comparatif/transactionnel), mot-clé cible et collection/produit à mailler. Rythme : 1 article/semaine.\n## 7. Maillage interne (tableau Source/Cible/Ancre optimisée/Impact)\n## 8. Stratégie mots-clés : courte traîne, transactionnels, longue traîne, informationnels, comparatifs, FAQ\n## 9. Plan 7 jours (chaque jour avec le CHEMIN SHOPIFY exact)\n## 10. Roadmap 30 jours (S1 technique, S2 pages business, S3 cocon de contenu, S4 consolidation + relance scan + Search Console)\n## 11. Routine SEO hebdomadaire récurrente\n## 12. Corrections prêtes à copier (2 meta premium, 2 titles, 3 FAQ, 3 alt, 3 ancres)\n## 🚀 Actions Orkestra (modules RÉELS uniquement : SEO Studio, Merchant Shield, AI Council, Code Shopify, Mémoire boutique)\n" +
    "INTERDIT : metas/titles faibles (« Achetez maintenant », « Livraison rapide », « Commandez dès aujourd'hui »). Préfère des metas premium orientées bénéfice (ex. « Découvrez nos … pensés pour … Livraison soignée. »). Pour le contenu de collection, précise toujours qu'il va dans la description de collection Shopify ou une section SEO du thème. Si tu recommandes un blog, fournis le calendrier éditorial complet (titres + intention + mot-clé + page à mailler).";
