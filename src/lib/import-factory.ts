...
export function buildTransformPrompt(products: ImportProductInput[], rules: ImportRules, mem: ImportMemory, ctx: ImportContext): string {
  ...
  // Titres (profil).
  const titleNat = "Le titre doit être NATUREL en français, PAS une traduction mot-à-mot des mots source : reformule, ajoute les connecteurs (en, à, effet…) — ex. « Lustre en Verre Effet Bulle », « Lustre Spirale en Cristal ». Capitalisation FRANÇAISE : majuscule au 1er mot et aux noms propres/marque, MINUSCULE aux connecteurs (en, de, à, et…) et après apostrophe (« Goutte d'eau », pas « Goutte d'Eau »). Pas de Title Case à l'anglaise, pas de mot anglais, pas de faute.";
  if (ctx.titleFormat === "brand_suffix" || (rules.titleFormat === "brand_suffix" && !ctx.titleFormat)) {
    directives.push(`Titres : format OBLIGATOIRE « Nom Produit | NomBrandé ». Le nom produit avant le « | » fait 3 à 6 mots. ${titleNat} Le NomBrandé est court, premium, mémorisable (~2 syllabes), UNIQUE, sans doublon ni quasi-doublon (éviter même début, accents inclus) — ce N'EST PAS le vendor « ${vendor} ». Noms brandés déjà pris : ${mem.brandNames.slice(0, 80).join(", ") || "(aucun)"}.`);
  } else {
